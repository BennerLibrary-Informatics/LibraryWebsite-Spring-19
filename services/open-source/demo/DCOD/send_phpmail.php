...
function email($to_name,$to_email,$from_name,$from_email,$subject,$message){
    $mail = new PHPMailer();
    $mail->IsSMTP(); // send via SMTP
    $return_email = $_SESSION['dcod_sys_email']; //Reply to this email ID
...
//    $mail->AddAddress($email,$name);
    if(strpos($to_email,','))
    {
        ...
        {
            $mail->AddAddress($email, $to_name[$x].$x);
//            $subject.=$x;
            $x++;
        }
    }else{
    ...
    $mail->AddReplyTo($return_email,$_SESSION['dcod_sys_admin']);
